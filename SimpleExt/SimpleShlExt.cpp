    ...
    LPCITEMIDLIST pidlFolder, LPDATAOBJECT pDataObj, HKEY hProgID )
{
	...
	if (pDataObj == nullptr) {
		m_cbFiles = 0;
		SHGetPathFromIDList(LPITEMIDLIST(pidlFolder), m_path);
		return S_OK;
	}
    ...
    UINT uidLastCmd, UINT uFlags )
{
	...
	// files or directories have been selected. find if in workspace
	if (m_cbFiles > 0) { // find where in the file system
		...
		// use the current working folder
		DWORD dwReturnCode = _tsplitpath_s(m_path, tcDrive, _MAX_DRIVE, tcDir, _MAX_DIR, tcFname, _MAX_FNAME, tcExt, _MAX_EXT);
		DWORD dwAttrib = GetFileAttributes(m_path);
		m_isOneDir = (dwAttrib & FILE_ATTRIBUTE_DIRECTORY);
	...
	m_nMenus = 0;
	if (m_cbFiles == 0) {
		// using the current working folder
		if (m_isOneDir) {
			if (m_isWorkspace) {
