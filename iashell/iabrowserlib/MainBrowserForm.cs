...
namespace iabrowserlib
{
    public partial class MainBrowserForm : Form
    {
        ...
        private void MainBrowserForm_Load(object sender, EventArgs e)
        {
            ...
            toolStripStatusCurrentAlbum.Text = "Album: none";
            toolStripStatusCurrentGallery.Text = "Gallery: none";
            ...
            AlbumManager albumManager = new AlbumManager(albumsPath, m_picturesPath);
            AlbumManager.LoadAlbums();
            Dictionary<string, AlbumInfo> albumList = albumManager.AlbumList;
            foreach (var key in albumList)
            {
                ListViewItem lvi = new ListViewItem(key.Value.Title);
                lvi.SubItems.Add(key.Value.Size.ToString());
                lvi.SubItems.Add(key.Value.LastUpdated);
        ...
        private async void GetUserPaths()
        {
            ...
            m_userspacePath = userSpace.Path;
            ...
            using (Graphics g = Graphics.FromImage((System.Drawing.Image)result))
            {
                ...
                else
                {
                    ...
                    if (OriginalAR >= TargetAR)
                    {
                        ...
                        float X = 0F;
                        float Y = ((float)szTarget.Height / 2F) -
                                  ((float)szTarget.Width / (float)bmpOriginal.Width * (float)bmpOriginal.Height) / 2F;
                    ...
                    else
                    {
                        ...
                        float X = ((float)szTarget.Width / 2F) -
                                  ((float)szTarget.Height / (float)bmpOriginal.Height * (float)bmpOriginal.Width) / 2F;
                        float Y = 0F;
        ...
        private void listViewAlbum_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            string title;

            ListView.SelectedListViewItemCollection selectedList = listViewAlbum.SelectedItems;
            bool ret = false;
            List<string> list = null;
            foreach (ListViewItem item in selectedList)
            {
                title = item.SubItems[0].Text;
                toolStripStatusCurrentAlbum.Text = "Album: " + title;
                //string fileName = Album.(title, ret);

                list = AlbumManager.getImageFileList(title);
            }

            processAlbumImages(list);
        }

        private void processAlbumImages(List<string> list)
        {
            ...
            if (list.Count != 0)
            {
                listViewAlbumImages.Items.Clear();
                imageListAlbumLarge.Images.Clear();
                System.Drawing.Image rawImage = System.Drawing.Image.FromFile(installPath + "\\config\\rawimage.jpg");
                ...
                foreach (string item in list)
                {
                        ...
                        ext == ".tif")
                    {
                        ...
                        imageListAlbumLarge.Images.Add(rimg);
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //imageListPicturesLarge.Images.Add(key, rawImage);
                            imageListAlbumLarge.Images.Add(rawImage);
                    ...
                    //lvi.ImageKey = key;
                    lvi.ImageIndex = count;
                    listViewAlbumImages.Items.Add(lvi);
        ...
        private void toolStripMenuItemCheckBox_Click(object sender, EventArgs e)
        {
            if (toolStripMenuItemCheckBox.Checked == true)
            {
                toolStripMenuItemCheckBox.CheckState = CheckState.Unchecked;
                listViewPictures.CheckBoxes = false;
            }
            else
            {
        ...
        private void toolStripMenuItemAddAlbum_Click(object sender, EventArgs e)
        {
            AddAlbumForm form = new AddAlbumForm();
            DialogResult dialogResult = form.ShowDialog();
            if (dialogResult == DialogResult.OK)
            {
                AlbumManager.Add(form.Title, form.Discription);
                AlbumInfo albumInfo = AlbumManager.GetAlbumInfo(form.Title);
                ListViewItem lvi = new ListViewItem(albumInfo.Title);
                lvi.SubItems.Add(albumInfo.Size.ToString());
